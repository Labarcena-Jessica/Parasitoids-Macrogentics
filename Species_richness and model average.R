...
abline(lm(sqrt(point_estimate$mean_by_cell_pi10) ~ sqrt(point_estimate$qD)), col = "red", lwd = 3)

lm_SR <- lm (sqrt(point_estimate$mean_by_cell_pi10) ~ sqrt(point_estimate$qD))
summary(lm_SR)
...
g<-(data_parasitoids_models$Species.Richness)


variablenames <- data.frame(names(data_parasitoids_models[,c(6:11, 13)]))
colnames(variablenames) <- "varname"

## response

y<-sqrt(data_parasitoids_models$mean_nuc.div) # square root of the mean nuc div per cell 
...
write.csv(as.data.frame(nd1), "nd1.csv")
...
plot((data_parasitoids_models$Temp_Range/10), sqrt(data_parasitoids_models$mean_nuc.div), xlab = "Temperature Range (°C)", ylab = "Mean Nucleotide Diversity per Cell", pch = 19, col = "dark grey")
abline(lm(sqrt(data_parasitoids_models$mean_nuc.div) ~  y+1), col = "red", lwd = 3)

y1 <- data_parasitoids_models$Temp_Range/10
lm_TR_nucdiv <- lm(sqrt(data_parasitoids_models$mean_nuc.div) ~ (data_parasitoids_models$Temp_Range) +1)

summary(lm_TR_nucdiv)

ggplot(data_parasitoids_models, aes(x = Temp_Range/10, y = sqrt(data_parasitoids_models$mean_nuc.div))) + 
  geom_point() +
  stat_smooth(method = "lm", col = "red") +
  theme_bw()
...
pdf("rel_import_nd1_J.pdf", width=3, height=6)
...
coords_t_lm <- as.matrix(data_parasitoids_models[, c(5,6)])

correlation_nucdiv_spsr <- modified.ttest(sqrt(data_parasitoids_models$Species.Richness), sqrt(data_parasitoids_models$mean_nuc.div), coords_t_lm)
